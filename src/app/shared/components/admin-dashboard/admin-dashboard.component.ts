...
export class AdminDashboardComponent implements OnInit {
  ...
  pendingItemType: 'user' | 'number' | 'all-users' | 'all-numbers' | null = null;
  ...
  confirmDelete(id: number, type: 'user' | 'number'): void {
    ...
    if (type === 'user') {
      this.confirmationTitle = this.texts.confirmation.deleteUserTitle[this.currentLanguageIndex];
      this.confirmationMessage = this.texts.confirmation.deleteUserMessage[this.currentLanguageIndex];
    } else {
      this.confirmationTitle = this.texts.confirmation.deleteNumberTitle[this.currentLanguageIndex];
      this.confirmationMessage = this.texts.confirmation.deleteNumberMessage[this.currentLanguageIndex];
    }
  ...
  confirmClearAll(type: 'users' | 'numbers'): void {
    ...
    if (type === 'users') {
      this.confirmationTitle = this.texts.confirmation.clearAllUsersTitle[this.currentLanguageIndex];
      this.confirmationMessage = this.texts.confirmation.clearAllUsersMessage[this.currentLanguageIndex];
    } else {
      this.confirmationTitle = this.texts.confirmation.clearAllNumbersTitle[this.currentLanguageIndex];
      this.confirmationMessage = this.texts.confirmation.clearAllNumbersMessage[this.currentLanguageIndex];
    }
    ...
    this.http.delete(`https://csms-backend-production.up.railway.app/${endpoint}/delete/${id}`).subscribe({
      next: () => {
        if (type === 'user') {
          ...
          this.phoneNumbers = this.phoneNumbers.filter(num => num.id !== id);
        }
      },
      error: (err) => {
